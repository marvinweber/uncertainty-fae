...
@torch.no_grad()
def bn_update_2(loader, model, cuda=True):
    r"""Updates BatchNorm running_mean, running_var buffers in the model.

    Implementation taken (and adjusted) from:
    https://github.com/pytorch/pytorch/blob/v1.12.1/torch/optim/swa_utils.py#L143..L196
    ...
    Args:
        ...
        model (torch.nn.Module): model for which we seek to update BatchNorm
            statistics.
        cuda (bool, optional): If set to true, input will be moved to cuda
            before being passed into the model (model must be on correct
            device already!).
    ...
    for input in tqdm.tqdm(loader, desc='batch norm update', total=len(loader), leave=False):
