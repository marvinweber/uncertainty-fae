...
class LitRSNABoneageLaplace(UncertaintyAwareModel, TrainLoadMixin):
    ...
    def forward_with_uncertainty(self, input) -> tuple[Tensor, Tensor, ForwardMetrics]:
        ...
        preds_mean, preds_var = self.la_model(
            input, pred_type="nn", link_approx="mc", n_samples=self.n_samples
        )
    ...
    def train_model(
        ...
        return TrainResult(
            interrupted=False, best_model_path=laplace_model_fpath, additional_info=additional_info
        )
