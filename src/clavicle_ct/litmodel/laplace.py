...
class LitClavicleLaplace(UncertaintyAwareModel, TrainLoadMixin):
    ...
    def load_model_from_disk(
        ...
        **kwargs,
        ...
        assert isinstance(model, LitClavicleLaplace), "Loaded model is not of correct instance!"
    ...
    def train_model(
        ...
        if not train_result_base_model.best_model_path or not os.path.isfile(
            train_result_base_model.best_model_path
        ):
            logger.error(
                "Base model training did not return valid best_model_path (%s)! Continue "
                "without loading best model weights!",
                train_result_base_model.best_model_path,
            )
        else:
            checkpoint = torch.load(train_result_base_model.best_model_path)
            assert "state_dict" in checkpoint, 'No "model_state_dict" in the base model checkpoint!'
        ...
        return TrainResult(
            interrupted=False,
            best_model_path=laplace_model_fpath,
